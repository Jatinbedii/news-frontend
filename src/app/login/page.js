  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.status === 200) {
        localStorage.setItem("newsappName", response.data.name);
        localStorage.setItem("newsappEmail", response.data.email);
        localStorage.setItem("newsappAdmin", response.data.isAdmin);
        router.push('/');
      } else {
